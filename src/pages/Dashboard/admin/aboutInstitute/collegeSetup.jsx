...
const findUserByIdentity = (users, candidate) => {
  const email = normalizeEmail(candidate?.email);
  const employeeId = normalizeText(candidate?.employeeId);

  return users.find((user) => {
    const sameEmail = email && normalizeEmail(user.email) === email;
    const sameEmployeeId =
      employeeId && normalizeText(user.employeeId) === employeeId;
    return sameEmail || sameEmployeeId;
  });
};
    ...
    <tr className={styles.subjectRow}>
      ...
      <td className={styles.td}>
        <select
          ...
          onChange={(e) =>
            updateField(["departments", dIdx, "courses", cIdx, "structure", semIdx, "subjects", sIdx, "teacher"], e.target.value)
          }
        >
        ...
        <div className={styles.courseBody}>
          <div className={styles.courseFields}>
            <Field label="Course Name" required>
              <input className={styles.input} placeholder="e.g. B.Tech "
                value={course.name}
                onChange={(e) => updateField(["departments", dIdx, "courses", cIdx, "name"], e.target.value)}
              />
            </Field>
            <Field label="Course Code">
              <input className={styles.input} placeholder="e.g. BTECH-IT"
                value={course.code}
        ...
        <div className={styles.tabContent}>
          <div className={styles.formGrid}>
            <Field label="Department Name" required>
              <input className={styles.input} placeholder="e.g. IT"
                value={dept.name}
          ...
          <div className={styles.hodBlock}>
            ...
            <div className={styles.formGrid} style={{ flex: 1 }}>
              <Field label="Full Name">
                <input className={styles.input} placeholder="Dr. raj" value={dept.hod.name}
...
export default function CollegeSetup() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const [inst, cfg, teachers, hods] = await Promise.all([
        getMyInstitute(),
        getInstituteConfig(),
      ...
      setAllHods(Array.isArray(hods) ? hods : []);
      ...
      setData({
        departments: (cfg?.departments || []).map((d) => ({
          ...
          hod: (() => {
            const matchedHod =
              (Array.isArray(hods) ? hods : []).find((hod) =>
                (hod.departmentId?._id || hod.departmentId) ===
                (d.department?._id || "")
              ) || d.hod;
            ...
            return {
              ...
              employeeId: matchedHod?.employeeId || "",
            };
          })(),
          teachers: (Array.isArray(teachers) ? teachers : [])
            .filter(
              (teacher) =>
                (teacher.departmentId?._id || teacher.departmentId) ===
                (d.department?._id || "")
            )
            .map((teacher) => ({
              _id: teacher._id,
  ...
  const addSubjectRow = (dIdx, cIdx, semIdx) => {
    setData((p) => {
      ...
      copy.departments[dIdx].courses[cIdx].structure[semIdx].subjects.push({
        _id: null, name: "", code: "", subjectType: "compulsory", teacher: "", _isNew: true,
      });
      return copy;
    });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const updatedDepts = [];
      const teacherDirectory = [...allTeachers];
      const hodDirectory = [...allHods];
      ...
      for (const dept of data.departments) {
        ...
        if (dept.hod?.email && dept.hod?.name && dept.hod?.employeeId) {
          try {
            ...
            if (!alreadyExists) {
              ...
              if (createdHod?.user?._id) hodDirectory.push(createdHod.user);
            }
          } catch (err) {
        ...
        const matchedHod =
          ...
          hodDirectory.find(
            (hod) =>
              (hod.departmentId?._id || hod.departmentId) === departmentId
          );

        await updateDeptHod({
          departmentId,
          hodId: matchedHod?._id || dept.hod?._id || null,
        }).catch(() => {});

        for (const teacher of dept.teachers) {
          if (!teacher.name || !teacher.email || !teacher.employeeId) continue;
          try {
            const existing = findUserByIdentity(teacherDirectory, teacher);
            const alreadyExists = Boolean(existing);
            if (!alreadyExists) {
              ...
              if (createdTeacher?.user?._id) teacherDirectory.push(createdTeacher.user);
            }
          } catch (err) {
            ...
            for (const sub of sem.subjects) {
              ...
              if (isNewSubject) {
                await addSubjectToConfig({
                  ...
                  subjectId,
                  structureNumber: sem.number,
                  teacherId: sub.teacher || null,
                }).catch(() => {});
              } else {
                await updateSubjectTeacher({
                  departmentId,
                  courseId,
                  structureNumber: sem.number,
                  subjectId,
